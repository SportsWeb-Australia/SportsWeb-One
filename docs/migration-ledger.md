...
| `compliance-completion.sql` | `compliance_documents` table + private `compliance-documents` storage bucket (WWCC evidence — `club-media` is public, this isn't) + FK on `compliance_records.document_id`; `compliance_risk_count()` and `compliance_alert_targets()` RPCs; `document_id` added to `get_member_detail`'s compliance arm; `club.compliance` auto-detection added to `club_setup_status()` + a matching `launch_step_catalog` row | ✅ APPLIED 2026-08-18 (applied via Supabase MCP `apply_migration`, function bodies re-verified with `pg_get_functiondef` before edit) |
| `compliance-alerts.sql` | weekly `pg_cron` schedule calling the `compliance-alerts` Edge Function | ⏳ NOT YET SCHEDULED — template has a `<SERVICE_ROLE_KEY>` placeholder Carson fills in and runs by hand (same pattern as `trial-nurture.sql`); the Edge Function itself is deployed |
| `compliance-check-types.sql` | rewrites `compliance_risk_count()` and `compliance_alert_targets()` to a full role → required-check-type matrix (WWCC, coach/trainer accreditation, first aid, official accreditation, safeguarding) instead of WWCC-only — mirrors `src/lib/complianceTypes.ts` | ✅ APPLIED 2026-08-18 (applied via Supabase MCP `apply_migration`, spot-checked against real person_roles data) |
| `compliance-club-requirements.sql` | `club_compliance_requirements` table (per-club override of the requirement matrix — turn a role/check-type requirement on or off per club) + rewrites `compliance_risk_count()`/`compliance_alert_targets()` again to merge platform defaults with a club's overrides | ✅ APPLIED 2026-08-18 (applied via Supabase MCP `apply_migration`; merge logic verified live — inserted a test override, confirmed `requirements` CTE output changed correctly, then deleted it) |
