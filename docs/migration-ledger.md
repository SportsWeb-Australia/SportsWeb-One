...
| `injury-concussion.sql` | Injury & concussion management. Drops the abandoned prototype `public.injuries` table (found live in prod during this build — its existence contradicted the handover doc's "green-field" claim; verified via direct introspection, 1 unattributed test row on Dookie United). Adds `injury_records` / `injury_stages` / `injury_stage_templates` / `injury_stage_template_items` / `injury_documents` / `injury_access_log`, the `can_access_injury()` / `injury_coach_team_ids()` RLS helpers (senior admins: full club; coaches/team managers: own team only, via `people.user_id` → `person_roles.team_id`), 9 SECURITY DEFINER RPCs, and the private `injury-documents` storage bucket. A follow-up migration revoked default `PUBLIC` execute/table grants the security advisor flagged (anon could technically call the SECURITY DEFINER RPCs, though internal `auth.uid()` checks already blocked real data access) | ✅ APPLIED 2026-08-18 (verified: 6 tables + bucket + template row present via introspection; anon/PUBLIC grants confirmed empty after the revoke) |
